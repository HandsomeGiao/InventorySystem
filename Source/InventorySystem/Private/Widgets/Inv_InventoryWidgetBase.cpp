...
void UInv_InventoryWidgetBase::InitializeInventory(int32 InTotalSlots, int32 InColumnsPerRow)
{
	...
	for (int32 i = 0; i < TotalSlots; ++i)
	{
		...
		{
			UE_LOG(LogInventorySystem, Error,
			       TEXT("UInv_PlayerInventoryWidget::InitializeInventory: Failed to create entry widget at index %d"),
			       i);
			continue;
		}
                                               ...
                                               int32 TargetSlotIndex)
{
	...
	if (!IsValid(RequestInventory))
	{
		...
		UInv_InventoryBase* LocalInventory =
			LocalPawn ? LocalPawn->FindComponentByClass<UInv_InventoryBase>() : nullptr;
