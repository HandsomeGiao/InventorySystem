...
FDataTableRowHandle UInv_VirtualItemDataSubsystem::CreateRowHandleByTag(const FGameplayTag& ItemTag) const
{
	...
	{
		UE_LOG(LogInventorySystem, Warning,
		       TEXT("Inv_VirtualItemDataSubsystem: Cannot create RowHandle, DataTable is not set!"));
		return Handle;
	}
...
void UInv_VirtualItemDataSubsystem::BuildTagToRowNameMap()
{
	...
	for (const FName& RowName : RowNames)
	{
		...
		{
			UE_LOG(LogInventorySystem, Warning,
			       TEXT("Inv_VirtualItemDataSubsystem: Row '%s' has invalid ItemTag, skipping"),
			       *RowName.ToString());
			SkippedCount++;
			continue;
		}
...
UInv_VirtualItemDataSubsystem* UInv_VirtualItemDataSubsystem::Get(const UObject* WorldContext)
{
	...
	{
		UE_LOG(LogInventorySystem, Warning,
		       TEXT("Inv_VirtualItemDataSubsystem::Get: Failed to get World from WorldContext"));
		return nullptr;
	}
