...
int32 FInv_ItemList::TryStackItem(const FInv_RealItemData& NewRealItem)
{
	...
	const FInv_MaxStackFragment* StackFragment = NewRealItem.GetVirtualItemData()->GetFragmentOfType<
		FInv_MaxStackFragment>();
	// 无法堆叠
	if (!StackFragment)
		return 0;
...
{
	// 线性搜索
	for (FInv_ItemFastArrayItem& Item : Items)
	{
		if (Item.GetItemId() == ItemId)
		{
			return &Item.RealItemData;
